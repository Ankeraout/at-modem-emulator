        ...
        if self._reading:
            if byte == 0x7e:
                if self._buffer_length != 0:
                    if self._frame_too_long:
                        self._frame_too_long = False

                    else:
                        self._receive_frame(
                            self._buffer[:self._buffer_length]
                        )
                ...
                if self._buffer_length == len(self._buffer):
                    self._frame_too_long = True
                
                else:
                    self._buffer[self._buffer_length] = byte
                    self._buffer_length += 1
            
    def _receive_frame(self, buffer: bytes) -> None:
        acfc = False

        if buffer[0:2] != b"\xff\x03":
            if not self._acfc:
                return
        ...
        if not self._check_fcs(buffer):
            self._fcs_error_count += 1
            return

        frame_start = 0 if acfc else 2
        frame_end = -4 if self._use_fcs_32 else -2
        
        self._receive_upper_protocol(buffer[frame_start:frame_end])
